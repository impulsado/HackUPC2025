...
def actualitzar_db(sender, appdata):
    ...
    timestamp_str = str(date["year"]+1900) + "-" + str(date["month"]) +"-"+str(date["month_day"]) + " " + str(hour) + ":" + str(min)+":00"
    timestamp = datetime.strptime(timestamp_str, '%Y-%m-%d %H:%M:%S').timestamp()
    print(timestamp)
    connection = sqlite3.connect("vueling.db")

    cursor = connection.cursor()
    try:
        sql = f'INSERT INTO vueling3 VALUES("{airline}",{flight_no},"{gate_char}",{gate_num},"{flag}",{timestamp})'
        print(sql)
        cursor.execute(sql)
        dpg.configure_item("afegit", show=True)
    except :
        sql2 = f'UPDATE vueling3 SET airline = "{airline}",gate_char ="{gate_char}",gate_num = {gate_num}, flags ="{flag}", epoch={timestamp} WHERE flight_no = {flight_no}'
        print(sql2)
...
with dpg.font_registry():
    ...
    default_font = dpg.add_font("42dot_Sans/42dotSans-VariableFont_wght.ttf", 18    )
